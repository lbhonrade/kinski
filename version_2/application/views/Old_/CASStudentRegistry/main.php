...
$category=array(array("CAS Student Registry","stdRegDiv","Add Student","StudentRegistry/add","Generate Report","1"),
				array("CAS Students' Delinquency History","stdDelinForm","Add Delinquency Case","DelinquencyHistory/add","Generate Report","2"),
				array("CAS Students' SDT History","stdSDTDiv","Add SDT Case","SDTHistory/add","Generate Report","3"));
?>
<style>
...
<script type="text/javascript">
	...
	function deleteData(callr,localID,categoryOperation){
		if(confirm("Are you sure you want to delete?")){
			$.post("<?php echo base_url();?>index.php/main/StdRegistryDB/"+categoryOperation,searchResult[localID],function(data){
				switch(categoryOperation){
					case 4:alert("Student Record deleted.");break;
					case 8:alert("Record deleted.");break;
					case 12:alert("Record deleted.");break;
				}
				callr.parent().parent().remove();
			},"html");
		}
	}
</script>
	...
	<?php foreach($category as $x){ ?>
		...
		<div id="<?php echo $x[1]; ?>">
				<div class="bor categories">
					...
					<form>
						<input type="button" class="addBtn fxnBtn" value="<?php echo $x[2]; ?>" onclick="loadFunctionUI($(this),<?php echo "'".$x[3]."'";?>,1,-1);" style="width:100%"/>
						<a href="<?php echo base_url();?>index.php/main/genPDFReport/<?php echo $x[5]; ?>"><input type="button" class="fxnBtn" value="<?php echo $x[4]; ?>" onclick="loadFunctionUI($(this),<?php echo "'".$x[3]."'";?>,1,-1);" style="width:100%"/></a>
					</form>
				</div>
