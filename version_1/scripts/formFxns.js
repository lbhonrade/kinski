	...
	for(i in formArray){
		switch(formArray[i]["type"]){
			...
			case 2:row=$("<tr></tr>").append($("<td>"+i+"</td>").css(labelStyle)).append("<td></td>");
				...
				break;
			case 3:row=$("<tr></tr>").append($("<td style='vertical-align:top;'>"+i+"</td>").css(labelStyle)).append("<td><span><input type='button' value='"+formArray[i]["btnValue"]+"' name='' onclick=\""+formArray[i]["clickFxn"]+"\"/></span><div id='"+formArray[i]["destDiv"]+"'></div></td>");
				   $("#"+destID+">table").append(row);
				break;
